/*$T boole/Boole2/SistemaSecuencial.h GC! 1.116 05/02/01 18:53:25 */
...
#ifndef SistemaSecuencialH
#define SistemaSecuencialH
...
#define AUTOMATA_CORRECTO_FALTA_TRANSICION	(4)
#define NO_SIMULANDO						(0)
#define INTERACTIVA							(1)
#define BATCH_PARADO						(2)
#define BATCH_ACTIVO						(3)
#define MOORE								(0)
#define MEALY								(1)
#define RECONVERSION_CORRECTA				0
#define NO_RECONVERTIBLE					1

extern int		grabaste;
extern int		Radio;

// Radio de los circulos
extern TColor	ObjSel;

// Color del Objeto Seleccionado
extern TColor	ObjMov;

// Color del Objeto Moviendose
extern TColor	ColFon;

// Color de Fondo
extern TColor	ColAct;

// Color de estado activado inicio modificado Ainara
extern int		conTabla;
extern int		numpaso;

extern bool		simulando;
...
public:
	...
	// Devuelve la coordenada Y de la flecha
        ...
        bool CombinableCon(FLECHA &f);
        bool CombinarCon(FLECHA &f);
};
...
public:
	...
	int			pertenece(int a);
	int			pertenece2(int a);
	void		particionEquivalencia(void);
	void 		tablaMinimaAutomataNoEspecif(void);        
	...
	AnsiString	CogerEntradaActual(void);
	AnsiString	CogerNombreCirculoActual(void);
	AnsiString	CogerNombreCirculoSiguiente(void);
	...
	void		ConvertirMooreAMealy(void);
	void		ConvertirMealyAMoore(void);
	int			ReconvertirMealyAMoore(void);

  bool AplicarEntrada(AnsiString &ent);
  void Clk();
  void Resetear(void);
  ...
  bool EsIncompletamenteEspecificado(void);
 ...
 */
private:
	bool		ReconversibleMealyAMoore(void);
};
